...
class HiGNN(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        #mol_vec = global_add_pool(x, batch).relu_() #ZLJ
        mol_vec_ = global_add_pool(x, batch) #ZLJ
        mol_vec = global_add_pool(x, batch).relu_() #ZLJ
        ...
        if self.brics:
            ...
            #fra_x = global_add_pool(fra_x, cluster).relu_() #ZLJ
            fra_x_ = global_add_pool(fra_x, cluster) #ZLJ
            fra_x = global_add_pool(fra_x, cluster).relu_() #ZLJ
            ...
            #fra_vec = self.cross_att((fra_x, mol_vec), mol_fra_index).relu_() #ZLJ
            fra_vec, _ = self.cross_att((fra_x, mol_vec), mol_fra_index, return_attention_weights=False) #ZLJ
            fra_vec = fra_vec.relu() #ZLJ
            ...
            mol_vec_ = mol_vec_[fra_batch] #ZLJ
            att = F.cosine_similarity(mol_vec_, fra_x_, dim=1), mol_fra_index #ZLJ

...
SPACE = {
    ...
    'DATA.BATCH_SIZE': hp.quniform('batch_size', low=16, high=128, q=8), #ZLJ
    #'DATA.BATCH_SIZE': hp.choice('batch_size', [16, 24, 32, 40, 48, 56, 64, 72, 80, 88, 96, 104, 112, 120, 128]), #ZLJ
}

INT_KEYS = ['MODEL.HID', 'MODEL.DEPTH', 'MODEL.SLICES', 'DATA.BATCH_SIZE'] #ZLJ
...
def hyperopt(cfg, logger):
    ...
    best_result = \
        min(results, key=lambda result: (-1) * result['mean_score']) #ZLJ

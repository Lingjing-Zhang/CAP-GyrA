...
def atom_attr(mol, explicit_H=False, use_chirality=True, pharmaco=True, scaffold=True):
    if pharmaco:
        mol = tag_pharmacophore(mol) #药效团原子标记1，其他标记0
    if scaffold:
        mol = tag_scaffold(mol) #骨架原子标记1，其余标记0
...
def tag_pharmacophore(mol):
    for fungrp, qmol in FunQuery.items():
        matches = mol.GetSubstructMatches(qmol) #返回匹配药效团的原子序号
        match_idxes = []
        for mat in matches:
...
class MolData(Data):
    ...
    def __inc__(self, key, value, *args, **kwargs):
        if key == 'cluster_index':
            return int(self.cluster_index.max()) + 1 #碎片数量
        else:
...
def load_dataset_random(path, dataset, seed, task_type, tasks=None, logger=None):
    ...
    # Seed randomness
    # 保留数据集后面0.2作为测试集不变 ZLJ
    random = Random(seed)
    #indices = list(range(len(pyg_dataset)))
    indices_1 = list(range(len(pyg_dataset))) #ZLJ
    random.seed(seed)
    #random.shuffle(indices)

    train_size = int(0.6 * len(pyg_dataset))
    val_size = int(0.2 * len(pyg_dataset))
    test_size = len(pyg_dataset) - train_size - val_size
    #indices_2 = indices_1[:(train_size + val_size)]
    indices_2 = []
    ...
    trn_id, val_id, test_id = indices_2[:train_size], \
                                          ...
                                          indices_1[(train_size + val_size):] #ZLJ
...
def scaffold_split(pyg_dataset, task_type, tasks, sizes=(0.8, 0.1, 0.1), balanced=True, seed=1, logger=None):
    ...
    for index_set in index_sets: #这样划分每种数据集的分子骨架不同 ZLJ
...
def build_dataset(cfg, logger):
    ...
    cfg.defrost()
    task_name = get_task_names(os.path.join(cfg.DATA.DATA_PATH, 'raw/{}.csv'.format(cfg.DATA.DATASET)))#LiGang

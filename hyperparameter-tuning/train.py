...
def train(cfg, logger):
    ...
    early_stop_cnt = 0
    best_epoch, best_score = 0, -100 #ZLJ
    ...
    for epoch in range(cfg.TRAIN.START_EPOCH, cfg.TRAIN.MAX_EPOCHS):
        ...
        # 5: Save best results.
        if cfg.DATA.TASK_TYPE == 'regression' and val_score > best_score:#ZLJ
    ...
    score = validate(cfg, model, criterion, val_loader, best_epoch, device, logger=logger, eval_mode=True) #ZLJ
...
# python ./hyperparameter-tuning/train.py --cfg ./configs/a_input_pro/a_input_pro.yaml --opts 'SEED' 3407 'MODEL.BRICS' True 'MODEL.F_ATT' True --tag seed_3407
